...
//	TestPathfinding();
	...
	if (useGravity) {
		Debug::Print("(G)ravity on", Vector2(10, 40), Vector4(1,0,0,1));
	}
	else {
	...
	float distance = 0;
/*	if (testNodes.size() > 0) {
		direction = testNodes[4] - rottt->GetTransform().GetWorldPosition();
		distance = sqrtf(pow(direction.x, 2) + pow(direction.y, 2) + pow(direction.z, 2));
		if (distance > 3)
			moveTo(dt);
		else
			stoppu(dt);
...
void TutorialGame::TestPathfinding() {
	...
	Vector3 startPos(210, -70, 110);
	Vector3 endPos(850, -70, 860);
...
void NCL::CSC8503::TutorialGame::moveTo(float dt)
{
	Vector3 direction;
	...
	direction = testNodes[4] - rottt->GetTransform().GetWorldPosition();
	acceleration += 5 * dt;
	if (acceleration > limitacceleration)
		acceleration = limitacceleration;
	direction *= (acceleration * dt);
	rottt->GetPhysicsObject()->AddForce(direction);
}

void NCL::CSC8503::TutorialGame::stoppu(float dt)
{
	Vector3 direction;
	breakacceleration += 5 * dt;
	if (breakacceleration > limitbreakacceleration)
		breakacceleration = limitbreakacceleration;
	direction =  rottt->GetTransform().GetWorldPosition() - testNodes[4];
	direction *= (breakacceleration * dt);
	rottt->GetPhysicsObject()->AddForce(direction);
}
...
void TutorialGame::InitCubeGridWorld(int numRows, int numCols, float rowSpacing, float colSpacing, const Vector3& cubeDims) {
	for (int x = 0; x < numCols; ++x) {
		for (int z = 0; z < numRows; ++z) {
			Vector3 position = Vector3((x+100) * colSpacing, cubeDims.y, (z*100) * rowSpacing);
			rottt = AddCubeToWorld(position, cubeDims, 1.0f);
		}
	}
...
void TutorialGame::updateRotationCube(float dt)
{
