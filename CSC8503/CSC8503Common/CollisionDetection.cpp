...
bool CollisionDetection::RayOBBIntersection(const Ray&r, const Transform& worldTransform, const OBBVolume& volume, RayCollision& collision) {
		 Matrix3 invTransform = worldTransform.GetInverseWorldOrientationMat();
			 ...
			 Ray tempRay(invTransform * (r.GetPosition() - worldTransform.GetWorldPosition()),
			 invTransform * r.GetDirection());
		
			bool collided = RayAABBIntersection(tempRay,
				worldTransform.GetWorldPosition(),
				 volume.GetHalfDimensions(), collision);
		
			 if (collided) {
			 collision.collidedAt =
				( worldTransform.GetWorldOrientation().ToMatrix3() * collision.collidedAt) + worldTransform.GetWorldPosition();
	...
	const CollisionVolume * volB = b->GetBoundingVolume();
	 ...
	 VolumeType pairType = (VolumeType)((int)volA->type | (int)volB->type);
		  ...
		  if (volA->type == VolumeType::AABB &&
		   ...
		   if (volA->type == VolumeType::Sphere &&
			   volB->type == VolumeType::OBB) {
			   collisionInfo.a = b;
			   collisionInfo.b = a;
			   return OOBBSphereIntersection((OBBVolume &)* volB, transformB,
				   (SphereVolume &)* volA, transformA, collisionInfo);
	...
	Vector3 boxSize = volumeA.GetHalfDimensions();
		 ...
		 Vector3 localPoint = delta - closestPointOnBox;
		  float distance = (localPoint).Length();
			 ...
			 collisionInfo.AddContactPoint(
				 closestPointOnBox + worldTransformA.GetWorldPosition(),
...
bool NCL::CollisionDetection::OOBBSphereIntersection(const OBBVolume & volumeA, const Transform & worldTransformA, const SphereVolume & volumeB, const Transform & worldTransformB, CollisionInfo & collisionInfo)
{
	Matrix3 invTransform = worldTransformA.GetInverseWorldOrientationMat();
	Vector3 boxSize = volumeA.GetHalfDimensions();
	Vector3 delta = invTransform * worldTransformB.GetWorldPosition() -
		invTransform * worldTransformA.GetWorldPosition();
	...
	if (distance < volumeB.GetRadius()) {// yes , we ’re colliding !
		collisionInfo.AddContactPoint(
			worldTransformA.GetWorldOrientation().ToMatrix3() * closestPointOnBox + worldTransformA.GetWorldPosition(),
			distance == 0.0f ? worldTransformA.GetWorldOrientation() * delta.Normalised() : worldTransformA.GetWorldOrientation().ToMatrix3() * localPoint.Normalised(),
			(volumeB.GetRadius() - distance)
		);
		return true;
